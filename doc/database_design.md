...
| Column | Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
| `building_key` | INTEGER | **PRIMARY KEY** | |
| `campus_key` | INTEGER | **FOREIGN KEY**, NOT NULL | |
...
| `building_key` | INTEGER | **FOREIGN KEY** NOT NULL | |
...
| `person_key` | INTEGER | **FOREIGN KEY** | Host |
...
| Column | Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
| `person_key` | INTEGER | **PRIMARY KEY**, **FOREIGN KEY** | Teacher |
| `course_key` | INTEGER | **PRIMARY KEY**, **FOREIGN KEY** (参考 `Course`) | |
...
* **设计体现 (`Preference` 表)**：该表通过 `user_key` 和 `room_key` 构成关联。其非主属性 `level` (偏好权重) 既不单单依赖于用户（同一个用户对不同房间评分不同），也不单单依赖于房间（同一个房间不同用户评分不同）。`level` 完全依赖于 `(user_key, room_key)` 这个联合主键，严格满足 2NF。
...
* **案例 A：空间域的层级解耦 (Campus -> Building -> Room)**
  * *常规错误设计*：在 `Room` 表中直接记录 `building_name` 和 `campus_name`。这会导致传递依赖 (`room_key` $\rightarrow$ `building_key` $\rightarrow$ `campus_name`)。如果某个校区改名，需要更新无数条房间记录。
  * *本系统设计*：`Room` 表仅保留 `building_key` (外键)。建筑名称和校区名称被严格隔离在各自的实体表中。当建筑或校区发生变更时，只需在 `Building` 或 `Campus` 表中修改一行数据，彻底消除了**更新异常**。
* **案例 B：人员与账户的解耦 (Person vs. User)**
  * *本系统设计*：将物理实体属性（`name`, `person_code`）归入 `Person` 表，将系统访问属性（`type`）归入 `User` 表，二者通过 `person_key` 关联。这种设计确保了“即便某个教授尚未注册本系统账号，其个人信息依然可以独立存在于系统中（被作为 `Course-Teacher` 的外键引用）”，彻底消除了**插入异常**。
